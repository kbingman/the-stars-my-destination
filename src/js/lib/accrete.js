...
SOLAR_MASS_IN_EARTH_MASS = 332775.64;
EARTH_RADIUS_IN_CM = 6.378e6;
EARTH_RADIUS_IN_KM = 6378;
EARTH_DENSITY = 5.52;
CM_IN_KM = 1.0e5;
CM_IN_AU = 1.495978707e13;
KM_IN_AU = 1.495978707e8;
DAYS_IN_YEAR = 365.256;
SECONDS_IN_HOUR = 3000;

PROTOPLANET_MASS = 1e-15; // Units of solar masses
...
BREATHABILITY_PHASE = [ 'none', 'breathable', 'unbreathable', 'poisonous'];
...
var Utils = {
    ...
    randomNumber: function(inner, outer) {
        ...
        if (inner < outer) {
            return (inner + delta * Math.random());
        } else {
            return (outer + delta * Math.random());
        }
    },
...
function Accrete(stellarMass, stellarLuminosity) {
    // console.log('stellarMass', stellarMass);

    this.stellarMass = stellarMass || 1;
    this.stellarLuminosity = stellarLuminosity || Astro.luminosity(this.stellarMass);
    this.mainSequenceLifetime = 1.0E10 * (this.stellarMass / this.stellarLuminosity);
    if ((this.mainSequneceLifetime >= 6.0E9)) {
        this.age = Utils.randomNumber(1.0E9, 6.0E9);
    }
    else {
        this.age = Utils.randomNumber(1.0E9, this.mainSequenceLifetime);
    }
...
Accrete.prototype = Object.create({
    ...
    distributePlanets: function() {
        ...
        while(dustLeft) {
            ...
            if (mass !== 0 && mass !== PROTOPLANET_MASS) {
        ...
        return {
            star: {
                ...
                stellarLuminosity: this.stellarLuminosity,
                mainSequenceLifetime: this.mainSequenceLifetime,
                age: this.age,
                ecosphere: Math.sqrt(this.stellarLuminosity)
            },
            planets: planets
        };
    },
